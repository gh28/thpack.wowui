function A.Frame.createContentBox(parent, config)
...
function CastBar.create()
    local f = A.Frame.createContentBox(UIParent, {
        width = 160,
        height = 2,
        borderSize = 1,
    });
    ...
    local iconFrame = A.createContentBox(f, {
        width = 32,
        height = 32,
        borderSize = 2,
    });
...
function CastBar.reset(self)
    self.casting = nil;
    self.channeling = nil;
end

function CastBar.onEvent(self, eventName, ...)
    if (event == "PLAYER_ENTERING_WORLD") then
        if (UnitIsUnit(self.unit, "player")) then
            -- am i casting?
            if (CastingInfo()) then
                CastBar.onEvent(self, "UNIT_SPELLCAST_START", self.unit);
            elseif (ChannelInfo()) then
                CastBar.onEvent(self, "UNIT_SPELLCAST_CHANNEL_START", unit);
            else
                CastBar.reset(self);
            end
        end
...
function CastBar.setUnit(self, unit)
    ...
    if (self.unit) then
        ...
        CastBar.onEvent(self, "PLAYER_ENTERING_WORLD");
    else
